...
//    }
    ...
    fun handleApp(bot: Bot, message: Message, config: Config) {
        val chatId = ChatId.fromId(message.chat.id)
        val presentText = "Открыть волшебную карту"
        ...
        val buttonUrl = InlineKeyboardButton.Url (
            text = presentText,
            url = "https://github.com/PalmaDae/AlchAndGags"
        )

        val buttonWebApp = InlineKeyboardButton.WebApp(
            text = presentText,
            webApp = WebAppInfo("https://github.com/PalmaDae/AlchAndGags")
        )
...
//                photo = TelegramFile.ByFile(logoFile),
                ...
                caption = textMessage,
                replyMarkup = InlineKeyboardMarkup.create(listOf(buttonWebApp))
            ).fold(
                {
                    println("Message sent successfully")
                    println(
